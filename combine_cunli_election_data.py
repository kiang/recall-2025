        ...
        try:
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                if 'COUNTYNAME' not in data or 'TOWNNAME' not in data or 'VILLNAME' not in data:
                    # Handle special case files without standard structure
                    if json_file.name in ['魚池鄉_新城村、共和村.json', '竹山鎮_硘磘里.json', '坪林區_石𥕢里.json']:
                        # Load the cunli key directly from the data
                        if 'cunli' in data:
                            key = data['cunli'].replace('_', '')
                            cunli_data[key] = data
                        continue
                    else:
...
def combine_data(cunli_data, election_data):
    ...
    for cunli_code, election_info in election_data.items():
        ...
        # Combine the data
        combined_record = {
            'VILLCODE': cunli_info.get('VILLCODE', ''),
            'COUNTYNAME': cunli_info.get('COUNTYNAME', ''),
            'TOWNNAME': cunli_info.get('TOWNNAME', ''),
            'VILLNAME': cunli_info.get('VILLNAME', ''),
            'election_zone': election_info['zone'],
